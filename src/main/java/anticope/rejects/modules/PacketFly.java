...
import anticope.rejects.mixin.PlayerPositionAccessor;
...
public class PacketFly extends Module {
    ...
    public void onSendMovementPackets(SendMovementPacketsEvent.Pre event) {
        ...
        speed =
                mc.player.input.playerInput.jump() && (checkCollisionBoxes || !(mc.player.input.movementForward != 0.0 || mc.player.input.movementSideways != 0.0)) ? (antiKick.get() && !checkCollisionBoxes ? (resetCounter(downDelayFlying.get()) ? -0.032 : verticalSpeed.get()/20) : verticalSpeed.get()/20) : (mc.player.input.playerInput.sneak() ? verticalSpeed.get()/-20 : (!checkCollisionBoxes ? (resetCounter(downDelay.get()) ? (antiKick.get() ? -0.04 : 0.0) : 0.0) : 0.0));
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof PlayerPositionLookS2CPacket && !(mc.player == null || mc.world == null)) {
            ...
            if (setYaw.get()) {
                ((PlayerPositionAccessor) event.packet).setPitch(mc.player.getPitch());
                ((PlayerPositionAccessor) event.packet).setYaw(mc.player.getYaw());
            }
            if (setID.get()) {
    ...
    private void sendPackets(double x, double y, double z, boolean teleport) {
        ...
        if (invalidPacket.get()) {
            packetSender(new PlayerMoveC2SPacket.PositionAndOnGround(outOfBoundsVec.x, outOfBoundsVec.y, outOfBoundsVec.z,
